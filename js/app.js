(async function() {
  ...
  async function renderAirports(filter = '') {
    ...
    if (filter) airports = airports.filter(a => [a.ICAO, a.AirportName].join(' ').toLowerCase().includes(filter.toLowerCase()));
    container.innerHTML = airports.map(a => `
      <div class="ref-card">
        ...
        ${a.GeneralNotes ? `<div class="ref-field"><span class="ref-label">Notes</span><p>${escapeHtml(a.GeneralNotes)}</p></div>` : ''}
      </div>`).join('') || '<div class="empty-state"><p>No airports loaded. Sync from Google Sheets.</p></div>';
  }

  async function renderHotels(filter = '') {
    const container = document.getElementById('hotels-list');
    let hotels = await db.getAllHotels();
    if (filter) hotels = hotels.filter(h => [h.HotelName, h.City, h.Location].join(' ').toLowerCase().includes(filter.toLowerCase()));
    container.innerHTML = hotels.map(h => `
      <div class="ref-card">
        ...
        ${h.Notes ? `<div class="ref-field"><span class="ref-label">Notes</span><p>${escapeHtml(h.Notes)}</p></div>` : ''}
      </div>`).join('') || '<div class="empty-state"><p>No hotels loaded. Sync from Google Sheets.</p></div>';
  }

  async function renderCities(filter = '') {
    const container = document.getElementById('cities-list');
    let cities = await db.getAllCities();
    if (filter) cities = cities.filter(c => [c.City, c.Country].join(' ').toLowerCase().includes(filter.toLowerCase()));
    container.innerHTML = cities.map(c => `
      <div class="ref-card">
        ...
        ${c.ShoppingAreas ? `<div class="ref-field"><span class="ref-label">Shopping</span><p>${escapeHtml(c.ShoppingAreas)}</p></div>` : ''}
      </div>`).join('') || '<div class="empty-state"><p>No cities loaded. Sync from Google Sheets.</p></div>';
  }
  ...
  function setupEventListeners() {
    ...
    document.getElementById('briefs-list').addEventListener('click', async e => {
      ...
      if (!card) return;
      const brief = await db.getBrief(card.dataset.id);
    ...
    document.getElementById('btn-clear-data').addEventListener('click', async () => {
      if (confirm('This will delete ALL local data. Are you sure?')) { await db.clearAll(); await renderAll(); toast('All local data cleared', 'warning'); }
    });
    ...
    document.getElementById('f-origin').addEventListener('change', async e => {
      ...
      if (airport) {
        ...
        if (!depProc.value && airport.DepProceduresTemplate) depProc.value = airport.DepProceduresTemplate;
  ...
  function populateForm(brief) {
    const fieldMap = [
      ['f-id', 'BriefsID'], ['f-flight', 'Flight_Number'], ['f-date', 'Date'],
      ['f-origin', 'Origin'], ['f-dest', 'Destination'], ['f-route', 'RouteKey'],
      ['f-sectors', 'Sectors'], ['f-type', 'FlightType'], ['f-dep-proc', 'DepProcedures'],
      ['f-dep-freq', 'DepFrequency'], ['f-dep-notes', 'DepNotes'], ['f-enroute', 'EnrouteNotes'],
      ['f-arr-rwy', 'ArrRunway'], ['f-taxi', 'TaxiRoute'], ['f-exp-taxi', 'ExpectedTaxi'],
      ['f-gate', 'Gate'], ['f-arr-freq', 'ArrFrequency'], ['f-apt-notes', 'AirportNotes'],
      ['f-transport', 'TransportTimeToHotel'], ['f-hotel', 'HotelName'],
      ['f-hotel-loc', 'HotelLocation'], ['f-hotel-amen', 'HotelAmenities'],
      ['f-hotel-notes', 'HotelNotes'], ['f-city', 'City'], ['f-country', 'CityCountry'],
      ['f-tz', 'Timezone'], ['f-todo', 'ThingsToDo'], ['f-shopping', 'ShoppingAreas'],
    ];
  ...
  function toast(message, type = '') {
    ...
    setTimeout(() => { el.style.opacity = '0'; el.style.transform = 'translateX(100%)'; el.style.transition = '0.3s ease'; setTimeout(() => el.remove(), 300); }, 300);
  }
...
**📄 `Index.html`** — add this one line in the `<head>`:
```html
...
Also: **clear your old IndexedDB** before testing — DevTools → Application → IndexedDB → delete `FlightBriefDB`, then reload.
